...
class Player : public Movement
{
    ...
    bool canClimbUp(Point position)
    {
        ...
        bool positionAllowsClimbing = gameBoard->isLadderAtPos(position) || (gameBoard->isLadderAtPos(position.oneBelow()) && gameBoard->isObstacleAtPos(position));
    ...
    bool canClimbDown(Point position)
    {
        ...
        bool positionAllowsClimbing = (gameBoard->isLadderAtPos(position) && !checkOnGround()) || gameBoard->isLadderAtPos(twoBelow);
        return positionAllowsClimbing && !midJump && !isFalling();
    }
